...
const EditCategory2Modal = ({ isOpen, onClose, onSuccess, category }) => {
  const [category2Code, setCategory2Code] = useState("");
  ...
  useEffect(() => {
    if (category) {
      setCategory2Code(category.category_2_code || "");
      setCategory2Name(category.category_2_name || "");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!category2Code.trim() || !category2Name.trim()) {
    ...
    try {
      const { error: updateError } = await supabase
        .from("categories_2")
        .update({
          category_2_code: category2Code,
          category_2_name: category2Name,
          status,
        })
        .eq("id", category.id);
              ...
              <DialogPanel className="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      id="edit-category2Code"
                      value={category2Code}
                      onChange={(e) => setCategory2Code(e.target.value)}
...
export default EditCategory2Modal;
