// src/components/Admin/Modals/Hotel/EditHotelModal.jsx
...
const EditHotelModal = ({ isOpen, onClose, onSuccess, hotelData }) => {
  const inputStyles =
  ...
  useEffect(() => {
    if (isOpen && hotelData) {
      setFormData({
        ...
        tin_number: hotelData.tin_number || "",
        ...
        country: hotelData.country || "Philippines",
        postal_code: hotelData.postal_code || "",
        social_media_links: hotelData.social_media_links || {
          facebook: "",
          instagram: "",
        },
      });

      // Populate address dropdowns based on existing data
      if (hotelData.country === "Philippines" && phAddressLib) {
        populatePhilippinesAddress(hotelData);
      } else if (hotelData.country && hotelData.country !== "Philippines") {
        populateInternationalAddress(hotelData);
      }
    } else if (isOpen && phAddressLib) {
      // If modal opens without data, load default regions for PH
      loadRegions();
    }
  }, [isOpen, hotelData]);
  ...
  const populatePhilippinesAddress = async (hotel) => {
    if (!phAddressLib) return;
    ...
    try {
      ...
      if (hotel.region) {
        const region = regions.find((r) => r.region_name === hotel.region);
        if (region) {
          ...
          // Load provinces for the found region
          const provinces = await phAddressLib.provinces(region.region_code);
          setAddressData((prev) => ({ ...prev, provinces: provinces || [] }));

          if (hotel.province) {
            const province = provinces.find(
              (p) => p.province_name === hotel.province
            );
            if (province) {
              setSelectedAddressCodes((prev) => ({
                ...prev,
              ...
              // Load cities for the found province
              const cities = await phAddressLib.cities(province.province_code);
              setAddressData((prev) => ({ ...prev, cities: cities || [] }));

              if (hotel.city) {
                const city = cities.find((c) => c.city_name === hotel.city);
                if (city) {
                  setSelectedAddressCodes((prev) => ({
                    ...prev,
                    city: city.city_code,
      ...
      console.error("Failed to populate Philippines address:", err);
      setError("Could not load address data. Please select manually.");
    }
  };

  const populateInternationalAddress = async (hotel) => {
    const countryInfo = Country.getAllCountries().find(
      (c) => c.name === hotel.country
    );
    ...
    try {
      ...
      if (hotel.province) {
        const stateInfo = states.find((s) => s.name === hotel.province);
        if (stateInfo) {
          setSelectedLocation((prev) => ({
            ...prev,
      ...
      console.error("Failed to load states for edit:", err);
    } finally {
      setInternationalAddressLoading((prev) => ({ ...prev, states: false }));
    }
  };
  ...
  const handleInputChange = (field, value) => {
    if (field === "country") {
      ...
      if (value === "Philippines" && phAddressLib) {
        loadRegions();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hotelData?.hotel_id) {
      setError("Error: Hotel ID is missing. Cannot save changes.");
      return;
    }
    ...
    try {
      const updateData = {
        ...
        social_media_links: Object.fromEntries(
          Object.entries(formData.social_media_links || {}).filter(
      ...
      const { data, error: updateError } = await supabase
        ...
        .update(updateData)
        .eq("hotel_id", hotelData.hotel_id)
        .select();
  ...
  const handleClose = () => {
    ...
    setError("");
    setAddressData({ regions: [], provinces: [], cities: [] });
    setSelectedAddressCodes({ region: "", province: "", city: "" });
    setInternationalAddressData({ states: [], cities: [] });
    setSelectedLocation({ countryCode: "", stateCode: "" });
    onClose();
