...
const EditCategory5Modal = ({ isOpen, onClose, onSuccess, category }) => {
  const [category5Code, setCategory5Code] = useState("");
  const [category5Name, setCategory5Name] = useState("");
  ...
  useEffect(() => {
    if (category) {
      setCategory5Code(category.category_5_code || "");
      setCategory5Name(category.category_5_name || "");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!category5Code.trim() || !category5Name.trim()) {
    ...
    try {
      const { error: updateError } = await supabase
        .from("categories_5")
        .update({
          category_5_code: category5Code,
          category_5_name: category5Name,
          status,
        })
        .eq("id", category.id);
              ...
              <DialogPanel className="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      id="edit-category5Code"
                      value={category5Code}
                      onChange={(e) => setCategory5Code(e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      id="edit-category5Name"
                      value={category5Name}
                      onChange={(e) => setCategory5Name(e.target.value)}
                      className="mt-1 w-full rounded-lg border border-gray-300 p-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
                      required
                    />
                  </div>
...
export default EditCategory5Modal;
