...
const EditCategory3Modal = ({ isOpen, onClose, onSuccess, category }) => {
  const [category3Code, setCategory3Code] = useState("");
  ...
  useEffect(() => {
    if (category) {
      setCategory3Code(category.category_3_code || "");
      setCategory3Name(category.category_3_name || "");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!category3Code.trim() || !category3Name.trim()) {
              ...
              <DialogPanel className="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      id="edit-category3Code"
                      value={category3Code}
                      onChange={(e) => setCategory3Code(e.target.value)}
...
export default EditCategory3Modal;
